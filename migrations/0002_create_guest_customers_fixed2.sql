...
ALTER TABLE orders ADD COLUMN guest_customer_id VARCHAR(255) NULL;
...
SET guest_customer_id = (
  SELECT gc.id FROM guest_customers gc 
  WHERE gc.email = JSON_UNQUOTE(JSON_EXTRACT(orders.delivery_address, '$.email')) 
  LIMIT 1
)
WHERE JSON_EXTRACT(delivery_address, '$.email') IS NOT NULL;
...
ALTER TABLE orders
  ADD CONSTRAINT fk_orders_guest_customer FOREIGN KEY (guest_customer_id) REFERENCES guest_customers(id);